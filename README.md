...
- **Automatic lifecycle**: Every host holds a `ViewModelBinding`. Reference counting drives disposal — when the last host releases its reference, the VM's `onDispose` fires. The VM's lazy `taskScope` ties unstructured Tasks to the same lifecycle and cancels active work on disposal.
...
`@MainActor` simplifies application state and interaction logic. AppleViewModel
keeps ViewModel state, bindings, the instance registry, reference counts,
notifications, and lifecycle transitions in one isolation domain, serializing
access to the UI-facing state and ownership graph. These operations are usually
short and directly serve UI on the main thread. Shared isolation reduces locks,
atomics, cross-actor synchronization, and `Sendable` plumbing, making synchronous
creation, notification, and disposal relationships easier to understand and maintain.

This does not mean expensive work belongs on the main thread. **Explicitly move
CPU-heavy computation and blocking work off the main thread**, then return to
MainActor to update state. A plain `Task {}` created in a MainActor-isolated
...
**An actor is an isolation domain, not a dedicated background thread.** Default
actors and detached tasks use the shared cooperative thread pool. Actors protect
background state but should not directly host long-blocking calls. APIs requiring
fixed thread affinity need a dedicated thread; a serial DispatchQueue does not
guarantee a fixed thread. See [Apple's responsiveness guide](https://developer.apple.com/documentation/xcode/improving-app-responsiveness)
and the [Actor execution model](https://developer.apple.com/documentation/swift/actor).

`nonisolated` does not guarantee background execution either. Synchronous methods
do not switch executors merely because they are nonisolated. In Swift 6.2, with
...
Background workers must not capture ViewModels, bindings, or their mutable state.
Manage ViewModel-owned tasks through `taskScope`, and make expensive computation
cooperate with cancellation. MainActor serialization does not make an entire async
method atomic: other tasks can change state across an `await`. Before applying a
result, check cancellation and, when relevant, whether the request is still current.
...
final class StatisticsViewModel: StateViewModel<StatisticsState> {
    ...
    func recalculate(values: [Int]) {
        ...
        taskScope.task { [weak self] in
            let total = await worker.value
            guard !Task.isCancelled else { return }
            self?.setState(StatisticsState(total: total))
        }
    }
}
```
...
Every ViewModel generation lazily owns one `ViewModelTaskScope`. Use it for
unstructured work whose result or side effects belong to that generation, such
as polling, `for await` listeners, refresh requests, or background calculations.
The scope is pure Swift and is disposed from `ViewModel.onDispose`; it does not
wait for object `deinit`, so cancellation still happens when another reference
temporarily keeps the disposed ViewModel object in memory.
...
final class FeedViewModel: StateViewModel<FeedState> {
    ...
    func startRefreshing() {
        taskScope.task { [weak self, client] in
            do {
                ...
                // Update feature error state when appropriate.
            }
        }
    }
}
...
Task lifetime follows these rules:

- Normal reference-count disposal cancels all active scope Tasks when the last
  owner releases a non-`aliveForever` ViewModel.
- Explicit `recycle` and `ViewModel.reset()` also cancel scope Tasks, including
  those owned by an `aliveForever` ViewModel.
- `aliveForever` deliberately skips zero-owner disposal, so its scope Tasks keep
  running until `cancelAll()`, `recycle`, or reset.
- Creating a scope Task after the ViewModel has already been disposed returns
  an immediately cancelled Task.

Cancellation is cooperative: reach a cancellation-aware `await`, call
`Task.checkCancellation()`, or inspect `Task.isCancelled` before applying a
result. Prefer `[weak self]`; a strong capture cannot prevent AppleViewModel's
logical disposal, but it can keep the Swift object in memory until the Task
actually responds to cancellation.

Use SwiftUI `.task` for work owned by a View's lifetime. A raw `Task {}` inside
a ViewModel is appropriate only when its lifetime is intentionally independent
from that ViewModel; otherwise create it through `taskScope`.
